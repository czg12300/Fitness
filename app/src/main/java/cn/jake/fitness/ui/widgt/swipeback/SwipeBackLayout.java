...
/**
 * 描述:处理了touch事件分发和处理的layout
 * 
 * @author zhj
 * @since 2014-7-3 下午3:06:59
 */
public class SwipeBackLayout extends FrameLayout {
	...
	public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		int shadowLeft = a.getResourceId(R.styleable.SwipeBackLayout_shadow_left, R.drawable.img_shadow_left);
		int shadowRight = a.getResourceId(R.styleable.SwipeBackLayout_shadow_right, R.drawable.img_shadow_right);
		int shadowBottom = a.getResourceId(R.styleable.SwipeBackLayout_shadow_bottom, R.drawable.img_shadow_bottom);
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			// 所有的向左滑动动作,或者是网上下拖动的幅度大于左右拖动的幅度时，不做处理
			if (detalX < 0 || Math.abs(detalY) > Math.abs(detalX)) {
				return false;
			}
			break;
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (changed) {
			// 获取SwipeBackLayout里面可以水平滑动的子view
	 ...
	 * @param parent
	 */
	private void getScrollChildView(List<ViewPager> mViewPagers, ViewGroup parent) {
